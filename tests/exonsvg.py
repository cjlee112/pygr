...
def spliceDraw(e, e1, spl, ysplStart, ysplHeight, transform):
	...
	return path(pd, stroke=splicecolor, stroke_width=0.5, fill='none')  #check if this stroke_width is ok
...
def exonDraw(e, y, exonheight, transform):                   #do I need to keep exonheight a variable?
	...
	return rect(x=xs0, y=y, width=xs1-xs0, height=exonheight,  fill='none', stroke='black')  
...
def draw(exons, gen_length, gen_start, file_name=None):
	"this func sets viewbox, calls scale fn and draws gene, default gene_start=0"
	d=drawing()   #open drawing
	s=svg((0,0,300,150),'30cm','15cm')   #create svg root element. Viewbox will be reset later in draw fns scaling calc
	...
	else: # HAND THE DRAWING OBJECT DIRECTLY BACK TO THE USER
		return d
