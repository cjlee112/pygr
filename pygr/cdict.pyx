...
class QueryMatchIterator(object):
    ...
    def __next__(self):
        if self.i>=len(self.qmlist.matches):
            raise StopIteration
        queryMatch=CGraphQueryMatch(self.qmlist.gqcompiler,self.qmlist.matches[self.i],
...
class QueryMatchList(object):
    ...
    def __getitem__(self,k):
        import types
        if isinstance(k,types.SliceType): # REWRAP JUST THE SPECIFIED SPLICE OF OUR LIST
            ...
            return CGraphQueryMatch(self.gqcompiler,self.matches[k],self.dataGraph)
    def get_more(self):
...
cdef class CDictionary:
    ...
    def __getitem__(self,k):
        ...
        e=cdict_getitem(self.d,self.key_index.key_index[k]) # raises KeyError if k not in key_index
        if e==NULL:
            raise KeyError
...
cdef class CGraphDict:
    ...
    def __new__(self,d,key_index):
        ...
        for k in l:
            self.d[0].dict[i].k=k
            self.d[0].dict[i].v=cdict_init(d[key_index.index_keys[k]],key_index)
            i=i+1
        self.d[0].n=i
    ...
    def __getitem__(self,k):
        ...
        e=cgraph_getitem(self.d,self.key_index.key_index[k]) # raises KeyError if k not in key_index
        if e==NULL:
            raise KeyError
        i=e-self.d[0].dict # GET INDEX BY SUBTRACTING POINTERS
        cd=CDictionaryRef(self,i) # REFERENCE HOLDER FOR PYTHON TO ACCESS THIS CDICT
        return cd
...
cdef class CDictIterator:
    ...
    def __init__(self,CDictionary cd,yieldKeys=1,yieldValues=0,CDictionaryRef cdr=None):
        if cdr is not None:
            self.cd=cdr
            self.d=cdr.d
...
cdef class CGraphIterator:
    ...
    def __next__(self): # PYREX USES THIS NON-STANDARD NAME!!!
        ...
        if self.yieldValues:
            cd=CDictionaryRef(self.g,self.i) # REFERENCE HOLDER FOR PYTHON TO ACCESS THIS CDICT
            if self.yieldKeys:            
                result=(k,cd)
            else:
                result=cd
